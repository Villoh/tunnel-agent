...
- **Cohesive alert system** (`Themes/Brushes.axaml`, `Themes/Controls.axaml`, `ProvidersView`, `MainWindow`): unified info / warning / error alert tonalities with solid (non-transparent) surfaces, larger padding and type, severity-colored icons and titles, and consistent light/dark brushes. The top-right provider status toast and the management-key-repaired toast now use this system (info by default, error tone with a warning icon for failures such as an invalid/unreachable provider URL), replacing the earlier semi-transparent tint.
