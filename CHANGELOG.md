...
- **Cohesive alert system** (`Themes/Brushes.axaml`, `Themes/Controls.axaml`, `ProvidersView`, `MainWindow`, `MainWindowViewModel`): unified info / warning / error alert tonalities with solid (non-transparent) surfaces, larger padding and type, severity-colored icons and titles, and consistent light/dark brushes. The top-right provider status toast and the management-key-repaired toast now use this system (info by default, error tone with a warning icon for failures such as an invalid/unreachable provider URL), replacing the earlier semi-transparent tint. The provider status toast is now pinned to the window corner (so it stays visible while the providers list scrolls) and auto-dismisses after a few seconds.
- **Unified status toasts** (`MainWindow`, `ConfigurationView`, `MainWindowViewModel`): all transient status toasts (provider status, management-key-repaired, and the engine/app “no update available” notices) now share the cohesive alert styling, stack in a single window-pinned top-right region so they stay visible regardless of which page is scrolled, and auto-dismiss. The “no update” toasts were moved out of the scrollable configuration view and now use dismiss commands instead of per-view code-behind handlers.
- **Update notification toasts restyled** (`MainWindow`): the bottom-center app/engine “update available” notifications now use the cohesive alert styling (solid info surface, severity icon, alert title/body) for consistency with the other toasts.
- **Toast fade in/out** (`Controls/ToastBehavior.cs`, `MainWindow`): added a reusable `ctrl:Toast.IsOpen` attached behavior that fades alerts/toasts in and out (animating opacity and collapsing from layout once hidden); all status and update toasts now use it instead of toggling `IsVisible` directly.
- **Configuration status toast unified** (`ConfigurationView`, `MainWindow`, `MainWindowViewModel`): the configuration status/error message (e.g. duplicate listen port) was moved out of the scrollable configuration view into the shared window-pinned alert stack (info/error tonality, severity icon, fade in/out, auto-dismiss), replacing the separate inline info banner and floating red error card.
