...
- **Linux/macOS: persistent user environment variables** (`UserEnvironmentService`): on Unix, `EnvironmentVariableTarget.User` has no persistent backing store — variables were silently lost on restart. A new `IUserEnvironmentService` abstraction (in `TunnelAgent.Abstractions`) now drives platform-specific implementations: `WindowsUserEnvironmentService` retains the existing registry + `WM_SETTINGCHANGE` behaviour; `UnixUserEnvironmentService` persists through two layers: (1) an app-owned shell-sourceable file (`$XDG_CONFIG_HOME/tunnelagent/environment`, with `export KEY=VALUE` lines) read at startup via `Initialize()`; (2) on Linux, a guarded block in `~/.profile` that sources that file — written once on first `Set`, removed automatically when all variables are cleared — making variables available to all login sessions (terminals and GUI apps) after the next login; on macOS, `launchctl setenv` propagates changes to the current GUI session immediately. The existing static `UserEnvironmentService` facade is preserved so all call sites are unchanged.
