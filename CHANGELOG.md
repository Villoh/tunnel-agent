...
- **Provider status toast error tone** (`ProvidersView`, `Themes/Controls.axaml`): the top-right provider status toast now shows an error tone (red tint, border, and warning icon) when the message is an error, such as an invalid/unreachable provider URL.
